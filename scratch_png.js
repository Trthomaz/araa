...
function parsePNG(filePath) {
  ...
  return { w, h, img, originalData: data };
}

function writePNG(outPath, w, h, img) {
  // Use filter type 0 (none) for all rows
  const stride = 1 + w * 4;
  ...
  console.log('Saved:', outPath, finalBuf.length, 'bytes');
}

const green = parsePNG('assets/pattern-fruits-green.png');
console.log('Green parsed:', green.w, green.h);

// Let's inspect green's pixels
let nonZeroAlpha = 0;
for (let i = 0; i < green.img.length; i += 4) {
  if (green.img[i + 3] > 0) nonZeroAlpha++;
}
console.log('Non zero alpha in green:', nonZeroAlpha, 'total:', green.w * green.h);
